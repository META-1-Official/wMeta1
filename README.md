...
npx hardhat run scripts/WrappedMetaDeploy.ts
TS_NODE_FILES=true npx ts-node scripts/WrappedMetaDeploy.ts
npx eslint '**/*.{js,ts}'
...
```shell
hardhat run --network ropsten scripts/WrappedMetaDeploy.ts
```
